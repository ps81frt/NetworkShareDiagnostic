...
try {
    ...
    # Diagnostiquer la vraie cause : "nom du fichier trop long" = ERROR_FILENAME_EXCED_RANGE
    # remonte par le pilote SMB quand LongPathsEnabled est desactive, PAS un probleme de chemin du script
    ...
    try {
        ...
        $longPathEnabled = [bool]$lp.LongPathsEnabled
    } catch {}
    $scriptPathLen = if ($PSCommandPath) { $PSCommandPath.Length } else { 0 }
    $conseil = if (-not $longPathEnabled) {
        'Cause probable : LongPathsEnabled desactive. Corriger : reg add HKLM\SYSTEM\CurrentControlSet\Control\FileSystem /v LongPathsEnabled /t REG_DWORD /d 1 /f'
    } elseif ($scriptPathLen -gt 160) {
        "Chemin du script trop long ($scriptPathLen cars) pour le chargement du module SmbShare. Placer le script dans C:\Temp\ et relancer."
    } else {
        "LongPathsEnabled est actif mais le module SmbShare ne se charge pas (chemin=$scriptPathLen cars). Verifier : Get-Module -ListAvailable SmbShare"
    }
    Write-Host "  [AVERT.] Get-SmbClientConfiguration inaccessible : $errMsg" -ForegroundColor Yellow
    Write-Host "           Bascule sur le registre LanmanWorkstation." -ForegroundColor DarkYellow
    Write-Host "           $conseil" -ForegroundColor DarkYellow

    $regPath   = 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanWorkstation\Parameters'
    $clientReg = Get-ItemProperty -Path $regPath -ErrorAction SilentlyContinue
    if ($clientReg) {
...
$SMBClientItems = if ($SMBClientConfig) {
    ...
    @([PSCustomObject]@{ Parametre='Erreur'; Valeur='Registre LanmanWorkstation inaccessible'; Risque='WARN'; Note='Relancer en Administrateur' })
}
...
$LocalAccounts = Set-Safe-Get {
    Get-LocalUser | ForEach-Object {
        [PSCustomObject]@{
            ...
            MdpRequis     = $_.PasswordRequired
            MdpExpire     = if ($_.PasswordExpires) { $_.PasswordExpires.ToString('yyyy-MM-dd') } else { 'N/A' }
            SID           = Set-Mask-SID $_.SID.Value
            Risque        = if ($_.Enabled -and -not $_.PasswordRequired) { 'CRITICAL' } elseif ($_.Enabled) { 'INFO' } else { 'OK' }
        }
    }
} @()
...
$ConnTests = foreach ($Target in $Neighbors) {
    try {
        ...
        [PSCustomObject]@{ Cible=$Target; Ping='N/A'; Port445='N/A'; UNC_IPC='Erreur'; Resultat='WARN' }
    }
}
...
$(Build-Section 'hosts' 'Fichier Hosts' '📝' (if ($HostsEntries) { Build-Table -ID 'tbl-hosts' -Data $HostsEntries -Columns @('IP','Hostname','Note') } else { "<p class='no-data'>Aucune entree personnalisee dans le fichier hosts</p>" }))

$(Build-Section 'connectivity' 'Tests de connectivite' '🧪' (if ($ConnTests) { Build-Table -ID 'tbl-conn-tests' -Data $ConnTests -Columns @('Cible','Ping','Port445','UNC_IPC','Resultat') -RiskColumn 'Resultat' } else { "<p class='no-data'>Aucun equipement voisin detecte</p>" }))
